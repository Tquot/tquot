...
export type SearchBookingHotelsResult = {
  hotels: HotelOption[];
  fallback: boolean;
  error?: string;
};
...
function createMockHotels(
  ...
  console.log("[search-booking] Creating fallback mock hotels", { city, hotelLevel });
...
async function rapidApiGet(url: string, rapidApiKey: string, label: string) {
  console.log(`[search-booking] ${label} request`, { url });
  ...
  console.log(`[search-booking] ${label} response`, {
  ...
  if (!response.ok) {
    ...
    console.error(`[search-booking] ${label} RapidAPI error`, {
      status: response.status,
      message,
      payload,
    });
...
export async function searchBookingHotels(
  input: SearchBookingHotelsInput,
): Promise<SearchBookingHotelsResult> {
  const destination = input.destination.trim();
  const checkIn = input.checkIn.trim();
  const checkOut = input.checkOut.trim();
  const rapidApiKey = input.rapidapiKey.trim();
  const propertyIds = input.propertyIds?.filter(Boolean);

  if (!rapidApiKey) {
    console.error("[search-booking] Missing RapidAPI key; returning fallback.");
  ...
  try {
    ...
    if (!locationMatch) {
      console.error("[search-booking] No location match found", {
        destination,
    ...
    console.log("[search-booking] Selected Booking.com location", {
    ...
    const hotels = filterHotelsByPropertyIds(
      normalizeHotelOptions(hotelPayload, nights),
      propertyIds,
    );

    if (hotels.length === 0) {
      console.error("[search-booking] RapidAPI returned no hotel options", {
        payload: hotelPayload,
        propertyIds,
      });

      return fallbackResult(
        "RapidAPI returned no hotel options after filters.",
        destination,
      );
    }

    return { hotels, fallback: false };
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Unexpected hotel search error.";

    console.error("[search-booking] Request failed", error);

    return fallbackResult(message, destination);
  }
}
