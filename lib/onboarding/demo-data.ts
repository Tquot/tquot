...
export { DEMO_SUGGESTION };

/**
 * Fechas relativas: el demo debe verse siempre futuro, no anclado a un año fijo.
 */
...
/** Parsed v2 shape for composeQuote / build stream. notes marks source=demo. */
export function buildDemoParsed(): ParsedTripInputV2 {
  const { arrivalDate, departureDate } = demoDates();
  const travelers = { adults: 2, children: [] as Array<{ age: number }>, infants: 0 };
